...
export async function GET(request: NextRequest) {
  ...
  const filters = [
    "ap.project_id=ANY($1::integer[])",
    "ap.appointment_type='site_visit'",
  ];
  ...
  if (status) {
    if (
      !SITE_VISIT_STATUSES.includes(
        status as (typeof SITE_VISIT_STATUSES)[number],
      )
    )
      return NextResponse.json(
        { error: "Invalid site visit status" },
        { status: 400 },
      );
    values.push(status);
  ...
  try {
    ...
    console.error("Failed to list site visits", error);
    return NextResponse.json(
      { error: "Unable to retrieve site visits" },
      { status: 500 },
    );
  }
}
...
export async function POST(request: NextRequest) {
  ...
  const validation = validateSiteVisitPayload(body, false);
  if (!validation.ok)
    return NextResponse.json(
  ...
  if (!canAccessProject(scope.context.access, data.project_id!))
    return NextResponse.json(
  ...
  try {
    ...
    if (data.assigned_to_user_id) {
      const conflict = await client.query(
        `SELECT appointment_id FROM appointments WHERE assigned_to_user_id=$1 AND status NOT IN ('cancelled','completed','no_show') AND tstzrange(starts_at,ends_at,'[)') && tstzrange($2::timestamptz,$3::timestamptz,'[)') LIMIT 1`,
        [data.assigned_to_user_id, data.starts_at, data.ends_at],
      );
      if (conflict.rowCount) {
        ...
        return NextResponse.json(
          {
            error:
              "The assigned user already has an appointment during this time",
            conflicting_appointment_id: conflict.rows[0].appointment_id,
          },
          { status: 409 },
        );
      }
    ...
    const result = await client.query(
      ...
      [
        ...
        data.organizer_user_id ?? scope.context.userId,
        data.assigned_to_user_id ?? null,
        data.assigned_to_team_id ?? null,
        scope.context.userId,
      ],
    );
    const appointment = result.rows[0];
    await client.query(
      `INSERT INTO site_visits (visit_id, arrival_instructions, transport_notes, attendee_count) VALUES ($1,$2,$3,$4)`,
      [
        appointment.appointment_id,
        data.arrival_instructions ?? null,
        data.transport_notes ?? null,
        data.attendee_count ?? null,
      ],
    );
    await client.query(
      `INSERT INTO site_visit_participants (visit_id, participant_type, user_id, participant_role, attendance_status) VALUES ($1,'user',$2,'organizer','confirmed') ON CONFLICT DO NOTHING`,
      [appointment.appointment_id, appointment.organizer_user_id],
    );
    if (appointment.contact_id)
      await client.query(
        `INSERT INTO site_visit_participants (visit_id, participant_type, contact_id, participant_role) VALUES ($1,'contact',$2,'attendee') ON CONFLICT DO NOTHING`,
        [appointment.appointment_id, appointment.contact_id],
      );
    ...
    await recordActivity(
      ...
      "appointment",
      "site_visit_scheduled",
      `Site visit scheduled: ${appointment.title}`,
      scope.context.userId,
      {
        metadata: {
    ...
    return NextResponse.json(
      {
        ...
        site_visit: {
          ...
          arrival_instructions: data.arrival_instructions ?? null,
    ...
    return NextResponse.json(
      { error: "Unable to create site visit" },
      { status: 500 },
    );
  } finally {
    client.release();
